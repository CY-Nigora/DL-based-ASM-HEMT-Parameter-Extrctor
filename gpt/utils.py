...
# ============================================================
# 1. Print to console + log file (tee)
# ============================================================
...
# ============================================================
# 2. Dropout-on-demand (inference-time MC dropout)
# ============================================================
@contextmanager
def dropout_mode(model: nn.Module, enabled: bool = False):
    """
    Enable dropout during eval/inference.
    """
    if not enabled:
        yield
        return
...
# ============================================================
def add_hparams_safe(
    base_writer: SummaryWriter, 
    run_dir: str, 
    hparams: Dict, 
    metrics: Dict
):
    """
    Record hyperparameters in JSON, safe from TB errors.
    """
...
# ============================================================
# 4. BoK flags (effective K for given epoch)
# ============================================================
def _bok_flags(cfg, phase: str, epoch: int):
    ...
    apply_set = set([
        s.strip().lower() 
        for s in (cfg.bok_apply or "train").split(",") 
        if s.strip()
    ])
    ...
    target = (cfg.bok_target or "sim").lower()
    bok_use_sim  = (target in ("sim", "both"))
    bok_use_meas = (target in ("meas", "both"))
