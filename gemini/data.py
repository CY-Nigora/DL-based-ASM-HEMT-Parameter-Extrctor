...
class DualArrayDataset(Dataset):
    """
    Returns Dictionary for training loop:
    {'x_iv': ..., 'x_gm': ..., 'y': ...}
    """
    ...
    def __getitem__(self, idx):
        ...
        # Return DICT to match training.py expectation
        return {
            "x_iv": torch.from_numpy(iv).unsqueeze(0),
            "x_gm": torch.from_numpy(gm).unsqueeze(0),
            "y": torch.from_numpy(y)
        }

class DualMeasDataset(Dataset):
    """
    Returns Dictionary for measurement loading:
    {'x_iv': ..., 'x_gm': ...}
    """
    ...
    def __getitem__(self, idx):
        return {
            "x_iv": torch.from_numpy(self.x_iv[idx]).unsqueeze(0),
            "x_gm": torch.from_numpy(self.x_gm[idx]).unsqueeze(0)
        }
...
def load_and_prepare_dual(data_path: str, cfg, PARAM_NAMES, PARAM_RANGE):
    ...
    # L_gm = int(np.prod(gm_shape)) # Unused explicitly here but good to know
    ...
    meta = dict(iv_shape=iv_shape, gm_shape=gm_shape, L_iv=L_iv)
